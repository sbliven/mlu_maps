...
def hexagon(center, apothem=spacing / 2 * 1.02, height=0, **attrs):
    ...
    pts = [
        [x * apothem + center[0], y * apothem * cos(camera_angle) / sqrt(3) + center[1]]
        for x, y in pts
    ]

    doc = svgwrite.container.Group()
    if height <= 0:
        doc.add(append_class(svgwrite.shapes.Polygon(pts, **attrs), "tile", "tile-top"))
    else:
        height_proj = height * sin(camera_angle)
        top = [[x, y - height_proj] for x, y in pts]
        doc.add(append_class(svgwrite.shapes.Polygon(top, **attrs), "tile", "tile-top"))
        doc.add(
            append_class(
                svgwrite.shapes.Polygon(top[:2] + [pts[1], pts[0]], **attrs),
                "tile",
                "tile-left",
            )
        )
        doc.add(
            append_class(
                svgwrite.shapes.Polygon(top[1:3] + [pts[2], pts[1]], **attrs),
                "tile",
                "tile-right",
            )
        )
...
def pentagon(center, side=spacing / sqrt(3), height=0, rotation=1, **attrs):
    ...
    pts = [
        (
            x0 + circumradius * cos(2 * pi * (2 * i + 1) / 10),
    ...
    if height <= 0:
        ...
        if 1 == rotation % 6:
            doc.add(
                append_class(
                    svgwrite.shapes.Polygon([top[1], top[2], pts[2], pts[1]], **attrs),
                    "tile",
            ...
            doc.add(
                append_class(
                    svgwrite.shapes.Polygon([top[2], top[3], pts[3], pts[2]], **attrs),
                    "tile",
                    "tile-left",
                    "tile-mid",
                )
...
def line_to_bb(pt, normal, xmin, ymin, xmax, ymax):
    ...
    if ny == 0:  # vertical
        ...
        crossings = {
            (x, y) for (x, y) in crossings if xmin <= x <= xmax and ymin <= y <= ymax
        }
        if len(crossings) == 2:
...
def make_grid(
    spacing, xmin, ymin, xmax, ymax, origin=[0, 0], major=1, minor=0, **line_attrs
):
    "Triangular grid"
    ...
    for y in itertools.chain(
        range(1, int(ymax // yspacing) + 1), range(0, int(ymin // yspacing) - 2, -1)
    ):
    ...
    for x in itertools.chain(
        ...
        endpoints = line_to_bb(
            (x0 + x * spacing, y0), (slope, -1), xmin, ymin, xmax, ymax
        )
        if endpoints:
            grid.add(svgwrite.shapes.Line(*endpoints, class_=class_, **attribs))
        endpoints = line_to_bb(
...
def make_level(config, level_name=0):
    ...
    for region in level["regions"]:
        ...
        for tile in region["tiles"]:
            ...
            center = (
                (x - y / 2) * spacing,
                (-y) * sqrt(3) / 2 * spacing * cos(camera_angle),
            )
            ...
            if sides == 5:
                geom = pentagon(
                    center,
                    class_=f'tile {region_id} {"t-"+svg_identifier(type_) if type_ else ""}'.strip(),
                    rotation=rotation,
                    **attrs,
                )
            else:
            ...
            for i, resource in enumerate(icons):
                ...
                insert = [
                    ...
                    center[1]
                    - size[1] / 2
                    - attrs.get("height", 0) * sin(camera_angle),
                ]
                if len(icons) == 1:
                    radius = 0
                elif 2 <= len(icons) < 4:
...
def main(args=None):
    parser = argparse.ArgumentParser(
        prog="mlu_map", description="Create a map from a yaml description"
    )
